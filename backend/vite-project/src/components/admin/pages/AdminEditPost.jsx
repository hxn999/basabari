...
import { faBolt, faBottleDroplet, faCalculator, faCheck, faElevator, faFire, faGasPump, faLocationDot, faPaperPlane, faPenToSquare, faPersonDress, faPersonMilitaryPointing, faTty, faUpload, faUser, faUsers, faVenus, faVideo, faWifi } from '@fortawesome/free-solid-svg-icons';
import { faCircleXmark } from '@fortawesome/free-regular-svg-icons';
import { useDispatch, useSelector } from 'react-redux';
// import { formActions } from '../../redux/slice/formSlice.js';
// import { AreaSelect } from '../components/postRent/areaSelect.jsx';
...
import { img,amenity } from '../../../assets/assets.js';
...
import { useAdminPostEditMutation, useGetSinglePostQuery } from '../../../../redux/slice/apiSlice.js';
...
export function AdminPostEdit() {
    ...
    let content = null
    const { data: post, error: postError, isSuccess: isPostSucc, isError: isPostErr, isLoading: isPostLoad } = useGetSinglePostQuery(_id)
    const [adminPostEdit, { data, error, isError, isLoading, isSuccess }] = useAdminPostEditMutation()

    let loadingUi = <div className='flex w-full mt-7  flex-col gap-5'>
        <Skeleton h={"h-10"} w={"w-[80vw]"} />
    ...
    useEffect(() => {
        ...
        post?.singlePost?.images.map((img) => {

            imageUrlToBase64(`http://localhost:3000${img}`)
                .then(base64String => { setImages((prev) => [...prev, base64String]) })
                .catch(error => console.log("-----------------------------------------------"));
        })

        setBed(post?.singlePost?.bed)
        setBath(post?.singlePost?.bath)
        setBalcony(post?.singlePost?.balcony)
        setLiving(post?.singlePost?.living)
        setDining(post?.singlePost?.dining)
        setFloorSize(post?.singlePost?.floorSize)
        setArea(post?.singlePost?.area)
        setAddress(post?.singlePost?.address)
        setDescription(post?.singlePost?.description)
        setRent(post?.singlePost?.rent)
        setType(post?.singlePost?.type)
        setRentDate(post?.singlePost?.rentDate)
        // setAmenities(post?.singlePost?.amenities)
        setFblink(post?.singlePost?.fbLink)
        setAdvance(post?.singlePost?.advance)
        setUtilityBills(post?.singlePost?.utilityBills)
        setLat(post?.singlePost?.lat)
        setLong(post?.singlePost?.long)
        if (post?.singlePost?.lat && post?.singlePost?.long) {
            setDisabled(true)
        }
    ...
    function handleSubmit(e) {
        ...
        if (isSuccess) {
            toast.success("Your post already has been Updated")
            return
        }
            ...
            adminPostEdit({
                ...
                postId:post?.singlePost?.postId,
                _id:post?.singlePost?._id,
                data:{
                    images,
                    bed,
                    bath,
    ...
    return (
        <>
            ...
            <section className="mx-auto max-w-7xl mt-2 px-8 ">
                ...
                {isPostLoad ? loadingUi : null}
                {isPostErr ? <Error text={'Profile Data Not Found !'} /> : null}



                <form onSubmit={handleSubmit} className={`${!isPostErr && !isPostLoad ? "flex" : "hidden"}  flex-col gap-7 mt-7`} >
                        ...
                        <div className='flex gap-4 max-[750px]:flex-col'>
                            ...
                            <div className='h-14 flex relative'>
                                ...
                                >
                                    ...
                                    {disabled ? "Click To Preview " : locationText}
                                    <dialog id="my_modal_x" className="modal">
                                        <div className="modal-box w-11/12 max-w-5xl">
                                            <h3 className="font-bold text-lg flex justify-between text-black">Your address <div onClick={reMaplocation} className='btn bg-green-500 hover:bg-green-700'> Get Location Again </div> </h3>
                                            <p className="py-4">Click the button below to close</p>
                    ...
                    <div className='flex justify-center flex-col'>
                        <div className='flex justify-center'>
                            ...
                            <button disabled={isLoading} type='submit' className='btn btn-md  bg-green-400 p-3 rounded-full px-5 hover:cursor-pointer hover:bg-green-600 text-base' >Save Edit {isLoading ? <span className="loading loading-spinner text-success"></span> : <FontAwesomeIcon icon={faPenToSquare} />}</button>
                        </div>
