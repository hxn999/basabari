...
export function PendPost() {
  const dispatch = useDispatch()
  let lol = useSelector(st => st.fetching)
  let content = null
  const { data, error, isError, isLoading, isSuccess } = useAdminPostQuery({ area: lol.area, lt: lol.lt, gt: lol.gt, bed: lol.bed, bath: 0, floor: lol.floor, isApproved: false })
  ...
  return (
    <div className='space-y-4 gap-4 w-full'>
      ...
      <PostFilter />
      <h3>{data?.post?.length? <span> <span className='text-red-500 font-semibold'>{data?.post?.length} </span>Pending post ...</span>: "No pending post"} </h3>
