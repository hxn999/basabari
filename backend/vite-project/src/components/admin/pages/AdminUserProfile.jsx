...
import { PostedCard } from '../../profile/PostedCard.jsx';
...
// import { usePfpPostQuery } from '../../redux/slice/apiSlice.js'
// import { LinearProgress } from '@mui/material'
import { Loading } from '../../ui/Loading.jsx';
import { Error } from '../../ui/Error.jsx';
import moment from 'moment'
// import { img } from '../assets/assets.js'
import { useSingleUserQuery } from '../../../../redux/slice/apiSlice.js';


export function AdminUserProfile() {
    // const dispatch = useDispatch()
    let params = useParams()
    const { data, isError, error, isLoading, isSuccess } = useSingleUserQuery({userId:params.userId,post:true})
    ...
    if(isSuccess&&data.evidence.length==0)
    {
        documents = <Error text={"User Have No Documents"}/>
    }
    if (isSuccess && data.posts.length == 0) {
        profilePost = <Error text={"User Did not Uploaded any Post"} />
            ...
            <section className='mx-auto max-w-7xl mt-8 px-8 '>
                ...
                <div className='flex gap-10 max-[900px]:flex-col w-full'>
                    ...
                    <div className='flex gap-10'>
                        ...
                        {/* -------------------------- profile picture ------------------------------------- */}
                        <img src={`http://localhost:3000/${data?.pfpSrc}`} alt="" className='w-32 h-32 object-cover max-[700px]:w-24 max-[700px]:h-24 rounded-full' />


                        <h1 className='text-2xl max-[800px]:text-base font-medium flex  flex-col pt-4 gap-5 '>
                            {/* ------------ name ------------------------------- */}
                            <div className='flex gap-5 items-center max-[800px]:gap-2 '>{data?.name}
                        ...
                        <div className='grid grid-cols-2 gap-10 max-[800px]:gap-5'>
                            {isLoading ? <Skeleton className='col-span-2' /> :
                                <p className='col-span-2 max-[800px]:text-xs'>
                                    <FontAwesomeIcon icon={faLocationCrosshairs} /> <span className='font-bold'> Address</span> : {data?.address}
                                </p>}
                            {isLoading ? <Skeleton className='' /> :
                                <p className='max-[800px]:text-xs'>
                                    <FontAwesomeIcon icon={faLocationCrosshairs} /> <span className='font-bold'> Area</span> : {data?.area}
                                </p>
                            }
                            {isLoading ? <Skeleton className='' /> :
                                <p className='max-[800px]:text-xs'>
                                    <FontAwesomeIcon icon={faPhone} /> <span className='font-bold'> Phone</span> : {data?.phone}
                                </p>
                            }
                        </div>
                    </div>
